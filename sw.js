// Service Worker — Consulta de Artigos v1.25.0
//
...
//   nova da app, muda-se este nome (ex: 'consulta-artigos-v1.25.0') — isso
...
//   não existe.
// - Nota (v1.25.0): esta cache do Service Worker guarda os FICHEIROS DA
...
const CACHE_NAME = 'consulta-artigos-v1.25.0';

const PRECACHE_URLS = [
  './consulta_artigos_v1.25.0.html',
  './manifest.json',
  './icon-192.png',
  './icon-512.png',
...
async function handleNavigation(request) {
  try {
    ...
    const cache = await caches.open(CACHE_NAME);
    const cached = await cache.match('./consulta_artigos_v1.25.0.html');
